...
class C(BaseConstants):
    NAME_IN_URL = 'payment_info_medium_diff'
    PLAYERS_PER_GROUP = 3
    NUM_ROUNDS = 1
...
class Survey(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        player.money_to_pay = int(participant.vars['Voting_medium_diff'][0])
        player.round_to_pay = int(participant.vars['Voting_medium_diff'][1])
        player.total_to_pay = int(participant.vars['Voting_medium_diff'][0])+5
