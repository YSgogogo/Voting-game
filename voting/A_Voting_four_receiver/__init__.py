...
doc = """
Three-player voting experiment with receive decisions+ full chat.
"""

class C(BaseConstants):
    NAME_IN_URL       = 'A_Voting_four_receiver'
    PLAYERS_PER_GROUP = 3
    NUM_ROUNDS        = 15
    AMOUNT_CORRECT    = 2
...
class StartRoundWaitPage(WaitPage):
    ...
    # ---------------------------------------------------------------------
    def _find_pattern(self, tag: str, others_tags: list[str], round_patterns: list[str]):
        for pat in round_patterns:
            if self._pattern_match(tag, pat, others_tags):
                return pat
        return f"{tag}+{''.join(others_tags)}1"

    # ---------------------------------------------------------------------
    # 3. Main after_all_players_arrive
    # ---------------------------------------------------------------------
    def after_all_players_arrive(self):
        # Shuffle players between groups every round
        self.subsession.group_randomly()

        # --- Session‑persistent signal table --------------------------------
        ...
        sv['pair_patterns'] = [random.choice(row) for row in pattern_rows]
        round_patterns: list[str] = sv['pair_patterns']

        # -------------------------------------------------------------------
        # Loop through each *current* group and select a record
        # -------------------------------------------------------------------
        for g in self.subsession.get_groups():
            # 1) For every player determine which patterns they still "need"
            ...
            for p in g.get_players():
                ...
                if not need_list:
                    need_list = round_patterns               # restart cycle
                needs[p.id_in_subsession] = need_list

            # 2) Find the *best* unused record from the signal table
            best_match: tuple[int, tuple, str] | None = None  # (idx, perm, state)
            ...
            # 3) Fallback: first unused record if nothing matches
            if not best_match:
                for idx, rec in enumerate(table):
            ...
            # 4) Apply the chosen record to the current group
            ...
            # 5) Log current pattern for each player
            for p in players:
                my_tag = p.signals + p.qualities
                others_tags = [pl.signals + pl.qualities for pl in players if pl.id_in_subsession != p.id_in_subsession]
        ...
        # Persist used indices for next round
    ...
    def after_all_players_arrive(self):
        for g in self.subsession.get_groups():
            ...
            if role == 'receiver' and 'do not receive' not in chosen.reveal_decision:
                if 'receive from all group members' in chosen.reveal_decision:
                    for snd in ps:
                        if snd!=chosen:
